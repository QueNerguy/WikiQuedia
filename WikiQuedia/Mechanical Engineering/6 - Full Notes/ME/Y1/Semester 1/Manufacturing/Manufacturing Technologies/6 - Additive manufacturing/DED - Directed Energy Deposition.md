...
### Process
In directed energy deposition (DED) a metal wire is deposited through a nozzle and molten with either a laser or an electron beam upon contact with the [workpiece](Manufacturing%20Technologies%20Overview#Terms%20and%20Disambiguation). The height of the table determines the height of the workheight of the laser or beamer, and the _nozzle_ moves in the horizontal plane. The nozzle can also be tilted change the angle from which the wire is fed[^dede].
##### Similar Processes
The process is similar to [fused deposition modeling](FDM%20-%20fused%20deposition%20modeling), in that it builds layers by feeding a wire to adhere to the previous layers, but suitable for metals only.
