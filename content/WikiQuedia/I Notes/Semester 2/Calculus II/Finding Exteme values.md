...
draft: false
---
Tags: [[calculus]], [[differentiation]], [[eigenvalues]], [[interval syntax]], [[vector math]]   <br>07-05-2025
...
[Saddle point](saddle%20points) can sometimes be found insteaad of an extreme value. To check wether a point is an extremum or just a saddle point, perform the _second derivative test_. There are two ways to perform the second derivative test:
...
To find the extreme values in a single-variable case we would realize that the derivative at that point is 0, so we take the derivative of the function, equal it to 0, and solve for $x$ to find the coordinate. <br>A similar process is the case for multivariable functions, where extremes can be reached in both directions, as seen in the graph for a saddle below. <br>![[Saddle_point_80%.png|450]]<br>By Nicoguaro - Own work, CC BY 3.0, [link](https://commons.wikimedia.org/w/index.php?curid=20570051)<br><br>The red dot depicted above is an example of a saddle point. At this point it is seen that the $x$ direction reaches a local maximum, while the $y$ direction reaches a local minimum. <br>To find these extremes, we take the [partial derivative](Partial%20Differentiation) to get the slope in both directions, and equal it to 0. <br>If we take the partial derivatives for the graph above we would get 2 equations, one for the partial derivative in $x$ direction, and one in $y$ direction. Observe that, when looking at the graph in the $x$ direction, the graph is made of _parabolas with a minimum_. For any slice going in $y$ direction of this 3D graph we get a 2D slice with height direction $z$, and length direction $y$. The parabola on this slice has it's _minimum_ at $y = 0$. By extension, when looking at the graph in $x$ direciton the 3D graph is made of parabolas that have their _minima_ at $y = 0$. With the same reasoning we can see that when looking at the graph in $y$ direction it's made of parabolas with their _maxima_ at $x = 0$. <br>To find the _critical point_ of this graph, we find the intersection of these lines that represent the partial derivatives. In this case that would be $(0, 0, 0)$, indicated in red.<br>In a general case, to find the _critical points_ of a function we find the coordinates of the point where both partial derivatives are 0. <br>In the case of the graph above however, we can see that at the critical point the $x$ direction has a _maximum_, while the $y$ direction has a _minimum_. As such this critical point is not an extremum, but a saddle point. This illustrates the importance of performing the [second derivative tests](#eliminating%20saddle%20points) outlined above.
