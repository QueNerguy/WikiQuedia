...
The *standard basis* for $n$-$t$ coordinates is made up of $\hat{\textbf{e}}_n$ and $\hat{\textbf{e}}_t$ for the $n$ and $t$ axes, respectively. <br><br>![[n-t_diagram_basis_80.png|400]]<br>source: *Engineering Mechanics Dynamics*, p57, 8th ed. 
...
We can use the thought process used [here](Modeling%20Rigid%20Body%20Motion#Angular%20Acceleration), but rewrite in terms of vectors $\vec{\pmb{\rho}}$ and $\vec{\pmb{\beta}}$. <br>Notice that the terms from the scalar equation above return in vector form. We see[^mrbacc]
$$
...
[^mrbveloc]: The thought process behind the ordering of these vectors can be found [here](Modeling%20Rigid%20Body%20Motion#Angular%20Velocity). Be aware that in this note, instead of $\rho$, $r$ is used, and in place of $\beta$, $\theta$. The time derivative $\dot{\beta}$ is denoted as $\omega$.
[^mrbacc]: The thought process behind the ordering of these vectors can be found [here](Modeling%20Rigid%20Body%20Motion#Angular%20Acceleration). Be aware that in this note, instead of $\rho$, $r$ is used, and in place of $\beta$, $\theta$. The time derivative $\dot{\beta}$ is denoted as $\omega$, and the double time derivative $\ddot{\beta}$ is denoted as $\alpha$.
