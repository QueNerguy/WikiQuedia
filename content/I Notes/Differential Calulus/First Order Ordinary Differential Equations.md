...
[^solve]: The solution to this is as follows: $$\frac{du}{dx} = u^2 + 1$$<br>We try to bring all variables to one side of the equation. We start by multiplying with $dx$ to obtain $$du = (u^2 + 1) dx$$<br>From here we divide by $u^2 + 1$ to isolate $dx$, yielding $$\frac{du}{u^2 + 1} = dx$$<br>Integrating both sides, gives us $$\int \frac{du}{u^2 + 1} = \int dx$$<br>$$\arctan(u) = x + C$$<br>$$u = \tan(x + C)$$<br>We can now substitute $x + y + 3$ for $u$, to get $$x + y + 3 = \tan(x + C)$$<br>Isolating $y$, our general solution is $$y = \tan(x + C) - x - 3$$
