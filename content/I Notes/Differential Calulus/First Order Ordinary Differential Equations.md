...
[^solve]: The solution to this is as follows: <br>$$\frac{du}{dx} = u^2 + 1$$<br>We try to bring all variables to one side of the equation. We start by multiplying with $dx$ to obtain<br>$$du = (u^2 + 1) dx$$From here we divide by $u^2 + 1$ to isolate $dx$, yielding<br>$$\frac{du}{u^2 + 1} = dx$$Integrating both sides, gives us<br>$$\int \frac{du}{u^2 + 1} = \int dx$$<br>$$\arctan(u) = x + C$$<br>$$u = \tan(x + C)$$<br>We can now substitute $x + y + 3$ for $u$, to get<br>$$x + y + 3 = \tan(x + C)$$<br>Isolating $y$, our general solution becomes<br>$$y = \tan(x + C) - x - 3$$
